...
from src.formatter import format_embeds
...
def run():
    ...
    embeds = format_embeds(results, errors, symbols, title)
    ...
    for embed in embeds:
        try:
            ...
            error_msg = f"[WARN] Peak-Drop 전송 실패: {e}"
...
if __name__ == "__main__":
    try:
        ...
        if webhook_url:
            try:
                send_discord(webhook_url, f"[ERROR] Peak-Drop 스크립트 오류: {e}")
            except Exception:
                pass
        sys.exit(1)
