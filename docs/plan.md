...
- [x] fetcher.py — FDR로 가격 조회
- [x] analyzer.py — 하락률 계산
- [x] formatter.py — 텍스트 리포트 생성
- [x] notifier.py — Discord 전송
- [x] main.py — 전체 파이프라인 연결
- [x] config/watchlist.yaml 작성
- [ ] GitHub Actions 설정
- [ ] 테스트 작성 및 통과 확인

### 2단계 (정보 확장)
- [ ] Discord Embed 형식으로 메시지 개선
- [ ] 하락률 구간별 색상/이모지 표시
- [ ] 전일 대비 변동률 추가
- [ ] 거래량 정보 추가
...
## 14. 향후 방향성
- **정보는 점진적으로 추가**: MVP에서는 현재가/고점/하락률만. 이후 변동률, 거래량, 뉴스 링크 등 단계적 확장
- **알림 조건 다양화**: 특정 하락률 도달 시 별도 경고, 신고가 갱신 알림 등
- **멀티 채널**: 디스코드 외 텔레그램, 슬랙 등 추가 가능한 구조로 notifier 설계
- **포트폴리오 뷰**: 전체 보유 종목의 가중 평균 하락률 요약
