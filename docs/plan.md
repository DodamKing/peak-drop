...
- **한국 주식 전일비**: KST 9:10/13:10 실행 시 장중이라 당일 시가를 별도 조회해 `(당일 시가 − 어제 종가) / 어제 종가`로 계산 (장 개장 기준 갭 반영). `current_price`(= 어제 종가)와 drawdown은 그대로 유지.
- **미국 주식 전일비**: ET 기준 장 마감 후 실행이라 `(어제 종가 − 그제 종가) / 그제 종가` 그대로.
...
## 12. 참고 사항
- **FDR 안정성**: FDR NaN 대응 완료 — yfinance fallback(US), NaN 시 에러 처리 (밀린 데이터 미사용)
